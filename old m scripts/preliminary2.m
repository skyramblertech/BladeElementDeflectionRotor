...
testlambda = 0:0.01:30;
testvel = omega*radius;
testdel = atan(testlambda/testvel);
testphi = 0.08;
testa = testdel-testphi;
test = testvel*sin(testa)+ ...
    testlambda.*(cos(testa)-1)- ...
    thrust/diskArea/density/testvel;
plot(testa,test);

index=1395;
testdl = testvel*(cos(testa(index))-1)-testlambda(index)*sin(testa(index));
